...
export const usePrometheus = (config: PrometheusTracingPluginConfig): Plugin => {
  ...
  if (errorsCounter) {
    for (const phase of (['parse', 'validate'] as const).filter(phase =>
      errorsCounter.phases.includes(phase),
    )) {
    ...
    for (const phase of (['execute', 'subscribe'] as const).filter(phase =>
      errorsCounter.phases.includes(phase),
    )) {
  ...
  const onParse: OnParseHook<{}> = ({ context, params }) => {
    ...
    return params => {
      ...
      for (const { handler } of phasesToHook.parse.filter(({ shouldHandle }) =>
        shouldHandle(args.params, context),
      ))
        handler(args);
    };
  };
  ...
  const onValidate: OnValidateHook<{}> = ({ context }) => {
    ...
    return ({ valid }) => {
      ...
      for (const { handler } of phasesToHook.validate.filter(({ shouldHandle }) =>
        shouldHandle(args.params, context),
      ))
        handler(args);
  ...
  const onContextBuilding: OnContextBuildingHook<{}> | undefined = ({ context }) => {
    ...
    for (const { handler } of phasesToHook.context.filter(({ shouldHandle }) =>
  ...
  const onExecute: OnExecuteHook<{}> = ({ args: { contextValue: context } }) => {
    ...
    function handleResult({ result }: { result: ExecutionResult }) {
      ...
      for (const { handler } of resultHandlers) handler(args);
    }

    const handleEnd = () => {
      const totalTime = (Date.now() - startTime) / 1000;
      const args = { params: fillLabelsFnParams, context, totalTime };
      for (const { handler } of endHandlers) handler(args);
    };
  ...
  const onSubscribe: OnSubscribeHook<{}> = ({ args: { contextValue: context } }) => {
    ...
    const handleError = ({ error }: { error: unknown }) => {
      ...
      for (const { handler } of errorHandlers) handler(args);
    };
