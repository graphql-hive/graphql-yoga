...
By default, GraphQL Yoga buffers each uploaded file into memory before making it available to
resolvers as a `File` object. For large files this can exhaust server memory.

You can enable **streaming mode** by setting `multipart: { stream: true }`. In this mode each file
variable is backed by a live `ReadableStream` coming directly from the network – no buffering occurs
...
On Node.js you can pipe the stream directly to disk using the Node.js streams interop:
