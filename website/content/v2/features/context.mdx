---
description:
  A context is usually created for each execution of a GraphQL Operation, and it is passed to the
  resolver functions as a third argument.
title: GraphQL Context
---

A context is usually created for each execution of a GraphQL Operation, and it is passed to the
resolver functions as a third argument. You can learn more about
[`Context` in GraphQL Tools docs](https://graphql-tools.com/docs/connectors).
...
- `request` - [Fetch API `Request` object](https://developer.mozilla.org/en-US/docs/Web/API/Request)
  ...
  accessing headers to authenticate a user
...
You can also pass a function as the `context` property. The function will be invoked for each
incoming GraphQL request with the initial context as an argument and after the invocation it will be
merged with the initial context.
...
For most users, the previous sections are already sufficient. If you, however, need to integrate
Yoga with a custom framework and/or want to attach runtime/framework-specific Request/Response
objects to the context. Also, if you are just curious about how Yoga works in depth, this might also
be interesting to you!

There are 4 steps that GraphQL Yoga follows to generate the final context object:

- [Default Context](#default-context) - this context is constructed for each incoming request and
  the same on all platforms
- [Server Context](#server-context) - the content of this context extension depends on the platform
  (Node.js/Deno/etc.) and the framework (raw Node.js/express/fastify)
- [User Context](#user-context) - this context is created for each incoming request with access to
  the default and server context
- [Envelop Plugin Context](#envelop-plugins) - the context from envelop plugins is processed last,
  the plugins have access to the default context, server context and user context
...
If you are using GraphQL Yoga as a standalone server with `.start()` method or exposing it as a
middleware as we show in our [express](/v2/integrations/integration-with-express) and
[Next.js](/v2/integrations/integration-with-nextjs) recipes.
...
favor `context.request`, as it is more future-proof (as Node.js HTTP servers adopt the Fetch
Response API).
...
[Fastify](/v2/integrations/integration-with-fastify) and
[koa](/v2/integrations/integration-with-koa), because of their custom request handling behavior. It
is not possible to use a basic Node.js
...
format. Also, note the generic definition in `createServer` for correctly typing the server context.
...
If you are using Cloudflare Workers with `@graphql-yoga/common` according to
[the integration guide](/v2/integrations/integration-with-cloudflare-workers), the WHATWG
[`FetchEvent`](https://developer.mozilla.org/en-US/docs/Web/API/FetchEvent) object will be added as
the server context object. That object is almost identical to what GraphQL Yoga provides you within
`YogaInitialContext`.
...
context). You can pass a factory function to `context` parameter of `createServer` configuration.
That function will receive the combination of the standard initial context and the server's (if
...
or extend it. See the documentation for [Envelop plugins](/v2/features/envelop-plugins) for more
information.
