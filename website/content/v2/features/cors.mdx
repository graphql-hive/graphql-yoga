...
1. **Avoid CORS by proxying your requests**

If you setup a proxy or redirect to forward requests from `example.com/api/*` to `api.example.com`,
you avoid CORS issues all together.

2. **Configure Yoga to send back CORS headers**

Yoga comes with CORS support out of the box - CORS can be configured when creating the server either
by passing a CORSOptions object, or a builder function that returns the CORSOptions object.
...
You can disable CORS on your Yoga server by simply passing `false` as the cors property. For
example::
