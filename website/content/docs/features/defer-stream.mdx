---
description:
  Use @defer and @stream with GraphQL Yoga. Stream and defer are directives that allow you to
  improve latency for clients by sending the most important data as soon as it's ready.
...
```sh title="curl command for executing @defer operation"
curl -g -X POST \
