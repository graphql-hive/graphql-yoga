...
```ts title="Response cache example" {14-17,22-25}
...
```ts title="Response Cache configuration based on  header"
useResponseCache({
...
```ts title="Response Cache configuration based on context"
useResponseCache({
  // cache based on the authentication header
...
```ts title="Response Cache configuration with scope using @cacheControl directive"
...
```ts title="Manual cache construction" {3,6}
...
```ts title="Create a custom Redis Cache"
...
```ts title="Use Graphql Yoga with Cloudflare KV Cache in Cloudflare Workers"
