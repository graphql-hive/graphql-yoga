...
```ts title="Execution Cancellation configuration" {1,6}
...
```graphql title="Simple GraphQL schema"
type Query {
  user: User
}
...
```graphql title="GraphQL Query for fetching user with best friend"
...
```bash title="Curl Request for executing the operation"
curl -g \
  -X POST \
...
```bash title="Server logs for canceled request"
DEBUG Parsing request to extract GraphQL parameters
DEBUG Processing GraphQL Parameters
INFO resolving user
...
```ts title="Altered server example with timer cleanup" {22-28}
import { createServer } from 'node:http'
import { createSchema, createYoga, useExecutionCancellation } from 'graphql-yoga'
import { Logger } from '@graphql-hive/logger'
...
```bash title="Server Logs after timer cleanup"
DEBUG Parsing request to extract GraphQL parameters
DEBUG Processing GraphQL Parameters
INFO resolving user
DEBUG Request aborted
...
```ts title="Resolver fetch cancellation example" {15}
