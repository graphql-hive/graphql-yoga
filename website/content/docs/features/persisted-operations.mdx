...
```ts title="Persisted operation setup"
...
```bash title="Execute persisted GraphQL operation"
...
```bash title="Arbitary GraphQL operation"
curl -X POST -H 'Content-Type: application/json' http://localhost:4000/graphql \
  -d '{"query": "{__typename}"}'
...
```json title="Example map extracted by GraphQL Code Generator"
{
  "ecf4edb46db40b5132295c0291d62fb65d6759a9eedfa4d5d612dd5ec54a6b38": "{__typename}",
...
```ts title="Use JSON file from filesystem as the persisted operation store"
...
```ts title="Urql Client Configuration" {2,8-13}
...
```ts title="Apollo Client Configuration" {2,4-6}
...
```ts title="Use parsed GraphQL documents as AST"
...
```ts title="Validate persisted operations"
const plugin = usePersistedOperations({
  //...
...
```ts title="Allow arbitrary GraphQL operations"
const plugin = usePersistedOperations({
...
```ts title="Relay Persisted Queries example"
...
```ts title="Extract persisted operation id from query parameters" {22-25}
import { createServer } from 'node:http'
import { createSchema, createYoga } from 'graphql-yoga'
import { usePersistedOperations } from '@graphql-yoga/plugin-persisted-operations'
...
```ts title="Extract persisted operation id from headers" {22-24}
import { createServer } from 'node:http'
import { createSchema, createYoga } from 'graphql-yoga'
import { usePersistedOperations } from '@graphql-yoga/plugin-persisted-operations'
...
```ts title="Extract persisted operation id from path" {10,23-25}
import { createServer } from 'node:http'
import { createSchema, createYoga } from 'graphql-yoga'
import { usePersistedOperations } from '@graphql-yoga/plugin-persisted-operations'
...
```ts title="Use external persisted operation store"
import { createServer } from 'node:http'
