...
```ts title="Error Masking Example"
...
```graphql title="GraphQL Operation"
query {
  greeting
}
```

```json title="Execution Result with leaking error message"
{
  "errors": [
    {
...
```ts title="Error thrown for non extisting user" {26-28}
...
```graphql title="Query for non existing user"
query {
  user(byId: "6") {
    id
  }
...
```json title="Execution result with error message" {4}
{
  "errors": [
    {
...
```ts title="GraphQLError with a variety of extensions" {5-13}
throw new GraphQLError(
  `User with id '${args.byId}' not found.`,
  // error extensions
  {
...
```ts title="Disable Error Masking"
...
For advanced use-cases e.g. when using GraphQL Yoga as a Gateway, you might to automatically expose
errors that are resolved from the subgraphs as you asume these already take care of hiding sensitive
information. In this case, you can customize the error masking behavior by providing a custom
`maskError` function.

Use the default `maskError` function as a fallback for all other errors.

```ts title="Custom Mask Error Usage"
