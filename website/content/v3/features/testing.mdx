...
<Callout>
  Calling the `yoga.fetch` method does not send an actual HTTP request. It basically simulates the
...
```ts title="Mutation and Query Operations"
...
```ts title="Subscription Operations"
import { parse } from 'graphql'
...
```ts title="TypeScript safe usage of executor"
import { buildHTTPExecutor } from '@graphql-tools/executor-http'
