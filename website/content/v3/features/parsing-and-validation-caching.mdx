...
title: Parsing and Validation Caching
---
...
```ts title="Disable validation and parser caching" {7-8}
...
```ts title="Custom cache store" {11-14,18-22}
import { createServer } from 'node:http'
...
Yoga uses [envelop](https://www.envelop.dev/)'s plugins
[`useParserCache`](https://www.envelop.dev/plugins/use-parser-cache) and
[`useValidationCache`](https://www.envelop.dev/plugins/use-validation-cache) underneath, read more
about them on their respective websites.
