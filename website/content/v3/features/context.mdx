...
```ts title="Context Usage Log Headers" {14}
...
```sh title="Execute operation and see logs in terminal"
...
```ts title="Returning function for context" {20}
import { createServer } from 'http'
import { createSchema, createYoga } from 'graphql-yoga'
...
```ts title="Returning promise for context creation" {17-19}
import { createServer } from 'http'
import { createSchema, createYoga } from 'graphql-yoga'
...
[express](/v3/integrations/integration-with-express) or
[Next.js](/v3/integrations/integration-with-nextjs) integration recipes.
...
[Fastify](/v3/integrations/integration-with-fastify) and
[koa](/v3/integrations/integration-with-koa), because of their custom non standard request handling
behavior. It is not possible to use a basic Node.js
...
```ts title="Typing request for Fastify integration" {2-3}
const yoga = createYoga<{
  req: FastifyRequest
  reply: FastifyReply
}>()
...
If you are using
[Cloudflare Workers according to the integration guide](/v3/integrations/integration-with-cloudflare-workers),
...
```ts title="User context factory function" {5-9}
import { createServer } from 'http'
...
or extend it. See the documentation for [Envelop plugins](/v3/features/envelop-plugins) for more
information.
