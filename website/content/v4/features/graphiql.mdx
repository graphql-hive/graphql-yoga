...
```ts title="Set the default Document String" {7}
...
[Learn more about disabling introspection](/v4/features/introspection)

</Callout>

```ts title="Disable GraphiQL" /graphiql: false/
import { createServer } from 'node:http'
import { createYoga } from 'graphql-yoga'
...
```ts title="Dynamic options based on the incoming request"
import { createServer } from 'node:http'
import { createYoga } from 'graphql-yoga'
...
```ts title="Render GraphiQL offline"
import { createYoga } from 'graphql-yoga'
