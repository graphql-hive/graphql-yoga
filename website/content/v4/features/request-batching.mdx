...
```bash title="Execute batched operation"
...
```ts title="Limit the amount of Batched Requests to 2"
import { createServer } from 'node:http'
import { createSchema, createYoga } from 'graphql-yoga'
...
```bash title="Execute batched operation (exceed limit)"
...
```json title="GraphQL Error response"
{
