...
```graphql title="src/schema/base/schema.graphql"
type Query {
  info: String!
...
```ts title="src/schema/base/resolvers/Query/feed.ts"
...
```ts title="src/schema.ts (previously)"
...
```ts title="src/schema/base/schema.mappers.ts"
import type { Link } from '@prisma/client'
...
```ts title="src/schema/base/resolvers/Link.ts"
...
```ts title="src/schema/base/resolvers/Comment.ts"
